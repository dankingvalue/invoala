...
import { defaultFlags, type FlagKey } from "@/lib/flags";
...
type PublicFlags = Pick<
  Record<FlagKey, boolean>,
  "aiComposer" | "maintenanceMode" | "signupPrompt" | "trustpilotStrip" | "proTeaser" | "emailCapture"
>;

export function HomeContent() {
  const initial = defaultFlags();
  const [flags, setFlags] = useState<PublicFlags>({
    aiComposer: initial.flags.aiComposer,
    maintenanceMode: initial.flags.maintenanceMode,
    signupPrompt: initial.flags.signupPrompt,
    trustpilotStrip: initial.flags.trustpilotStrip,
    proTeaser: initial.flags.proTeaser,
    emailCapture: initial.flags.emailCapture,
  });
  ...
  useEffect(() => {
    fetch("/api/flags-public")
      ...
      .then((data: { flags?: Partial<PublicFlags>; announcement?: string } | null) => {
        if (data?.flags) setFlags((f) => ({ ...f, ...data.flags }));
        if (typeof data?.announcement === "string") setAnnouncement(data.announcement);
      })
      .catch(() => {});
    fetch("/api/auth/me")
      .then((r) => (r.ok ? r.json() : null))
      .then((data: { user?: unknown } | null) => setLoggedIn(!!data?.user))
      .catch(() => {});
  }, []);
  ...
  return (
    <div id="top" className="scroll-mt-20">
      <HomeNav />
      <main>
      {loggedIn || !flags.signupPrompt ? null : <SignupPrompt />}
