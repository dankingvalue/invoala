...
// Pure presentational nav — takes user/impersonator as props instead of
// fetching them itself, so it can be reused by both a server-rendered
// caller (Nav, cookie-dependent, used on the dashboard where dynamic
// rendering is already unavoidable) and a client-rendered one (HomeNav,
// used on the static homepage — see components/HomeNav.tsx for why).
export function NavShell({
  ...
  return (
    ...
    {user && impersonatorEmail ? (
      <ImpersonationBar targetEmail={user.email} adminEmail={impersonatorEmail} />
    ) : null}
    </>
  );
}
