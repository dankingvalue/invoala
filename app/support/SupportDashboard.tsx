...
export function SupportDashboard({ myId }: { myId: string }) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <StatCard label="Users" value={stats.users.toLocaleString()} sub={`+${stats.newUsersInRange} this week`} />
        ...
        <StatCard label="Emails" value={stats.emailsInRange.toLocaleString()} />
      </div>
...
function MessagesTab({ myId }: { myId: string }) {
  ...
  const sendReply = async (e: FormEvent) => {
    ...
    const data = await res.json();
    if (data.ok) { setReply(""); setSuggestion(""); setIsInternalNote(false); setMacroId(undefined); loadConversation(selectedConv); }
    setSendingMsg(false);
  };
    ...
    return (
      <Panel>
        <div className="flex max-h-[calc(100vh-120px)] flex-col">
          ...
          <ConversationHeaderExtras conv={selectedConv} myRole="support" myId={myId} incident={incident} onUpdated={() => loadConversation(selectedConv)} />
