...
export async function POST(req: Request) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, 'ai', ?, ?, ?)
  `).run(convId, user.id, subject || "Support request", now, now);

  db.prepare(`
    INSERT INTO messages (id, conversation_id, sender_type, sender_id, content, created_at)
    VALUES (?, ?, 'user', ?, ?, ?)
  `).run(randomUUID(), convId, user.id, content, now);
  ...
  if (aiResponse.escalate) {
    ...
    db.prepare("UPDATE conversations SET status = 'escalated', updated_at = ? WHERE id = ?").run(now, convId);
    
    db.prepare(`
      INSERT INTO messages (id, conversation_id, sender_type, sender_id, content, created_at)
      VALUES (?, ?, 'system', NULL, ?, ?)
    `).run(randomUUID(), convId, "This conversation has been escalated to our support team. A team member will respond shortly.", now + 1);
    ...
    db.prepare(`
      INSERT INTO messages (id, conversation_id, sender_type, sender_id, content, created_at)
      VALUES (?, ?, 'ai', NULL, ?, ?)
    `).run(randomUUID(), convId, aiResponse.message, now + 1);
  }

  db.prepare("UPDATE conversations SET updated_at = ? WHERE id = ?").run(now + 1, convId);
