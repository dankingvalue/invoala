...
export async function POST(req: Request) {
  ...
  const db = getDb();
  const exists = db.prepare("SELECT id FROM users WHERE email = ? AND id != ?").get(newEmail, user.id);
  if (exists) {
    return Response.json({ error: "This email is already in use." }, { status: 409 });
  }

  db.prepare("UPDATE users SET pending_email = ? WHERE id = ?").run(newEmail, user.id);
  const token = issueToken(user.id, "email_change", 24 * 60 * 60e3, newEmail);
